...
def main():
    ...
    random_search = RandomizedSearchCV(estimator=SklearnECModel(), param_distributions=param_distributions, scoring=scorer, cv=cv, n_iter=50)

    print("Starting Random Search...")
    start_time = time.time()
    print(f"Start time: {datetime.fromtimestamp(start_time).strftime('%Y-%m-%d %H:%M:%S')}")
    
    random_search.fit(traindataset, train_label)
    print("Random Search complete.")
    
    end_time = time.time()
    elapsed_time = end_time - start_time
    print(f"Random Search complete. Time taken: {elapsed_time:.2f} seconds.")
