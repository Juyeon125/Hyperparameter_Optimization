...
def main():
    ...
    param_space = {
        ...
        'dropout_prob': Real(0.1, 0.5),
        'momentum': Real(0.8, 0.99),
        'epsilon': Real(1e-8, 1e-2, prior='log-uniform'),
        'batch_size': Integer(32, 128),
        'epochs': Integer(30, 30)
    }
    ...
    bayes_search = BayesSearchCV(estimator=SklearnECModel(), search_spaces=param_space, scoring=scorer, cv=cv, n_iter=50, random_state=Config.RANDOM_STATE)

    print("Starting Bayesian Optimization...")
    start_time = time.time()
    print(f"Start time: {datetime.fromtimestamp(start_time).strftime('%Y-%m-%d %H:%M:%S')}")
    
    bayes_search.fit(traindataset, train_label)
    print("Bayesian Optimization complete.")
    
    end_time = time.time()
    elapsed_time = end_time - start_time
    print(f"Bayesian Optimization complete. Time taken: {elapsed_time:.2f} seconds.")
