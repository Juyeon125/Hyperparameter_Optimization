...
def evaluate(params):
    ...
    model = SklearnECModel(learning_rate=learning_rate, dropout_prob=dropout_prob, momentum=momentum, epsilon=epsilon, batch_size=int(batch_size), epochs=int(epochs))
    cv = StratifiedKFold(n_splits=3)
    f1_scores = []
    for train_idx, val_idx in cv.split(traindataset, train_label):
        train_data = DataLoader([traindataset[i] for i in train_idx], batch_size=int(batch_size), pin_memory=True, num_workers=1, drop_last=True)
        val_data = DataLoader([traindataset[i] for i in val_idx], batch_size=int(batch_size), pin_memory=True, num_workers=1, drop_last=True)
...
def main():
    ...
    bounds = [(1e-5, 1e-1), (0.1, 0.5), (0.8, 0.99), (1e-8, 1e-2), (32, 128), (30, 30)]

    start_time = time.time()
    print(f"Start time: {datetime.fromtimestamp(start_time).strftime('%Y-%m-%d %H:%M:%S')}")
    
    result = differential_evolution(evaluate, bounds, strategy='best1bin', maxiter=10, popsize=15, tol=0.01)
    
    end_time = time.time()
    elapsed_time = end_time - start_time
    print(f"Differential Evolution optimization complete. Time taken: {elapsed_time:.2f} seconds.")

    best_params = {
        'learning_rate': result.x[0],
        'dropout_prob': result.x[1],
        'momentum': result.x[2],
        'epsilon': result.x[3],
