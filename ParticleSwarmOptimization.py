...
def main():
    ...
    lb = [1e-5, 0.1, 0.8, 1e-8, 32, 30]
    ub = [1e-1, 0.5, 0.99, 1e-2, 128, 30]

    start_time = time.time()
    print(f"Start time: {datetime.fromtimestamp(start_time).strftime('%Y-%m-%d %H:%M:%S')}")
    
    xopt, fopt = pso(evaluate, lb, ub, swarmsize=20, maxiter=10)
    
    end_time = time.time()
    elapsed_time = end_time - start_time
    print(f"PSO optimization complete. Time taken: {elapsed_time:.2f} seconds.")
